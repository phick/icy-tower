...
namespace Engine {
	...
	void PauseState::initButtons() 
	{
		...
		this->_data->asset.LoadTextures("_pauseBackground", "f.png");
	...
	void PauseState::initText() {
		...
		_resumeText.setPosition(_resumeButton.getPosition().x - 50, _resumeButton.getPosition().y -_resumeText.getGlobalBounds().height * 1.5);
		...
		_exitText.setPosition(_exitButton.getPosition().x, _exitButton.getPosition().y -_exitText.getGlobalBounds().height * 1.5);


		_barText.setFont(this->_data->asset.GetFont("arial"));
		_barText.setFillColor(sf::Color::Color(255, 0, 0));
		_barText.setString(" || GAME PAUSED ||");
